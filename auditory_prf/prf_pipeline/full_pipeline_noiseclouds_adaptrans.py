"""full_pipeline_noiseclouds_adaptrans.py
===========================================
pRF pipeline for noise-cloud (Gaussian filterbank) stimuli, WITH AdapTrans.

Same Phase 1 / Phase 2 run-assembly architecture as
full_pipeline_noiseclouds_notemporal.py, but:
  - Power-law sharpening is applied to the full population PSTH before
    chunking into tone-ON windows (not to the per-chunk mean rates
    afterward), matching full_pipeline_toneclouds_adaptrans.py.
...
def run_pipeline(
        ...
        total_run_dur_s: float = 660.0, # 8x3 conditions: opening_blank_s (10.0) + 32 trials x 20s (640) + closing_blank_s (10.0)
        # HRF
    ...
    _output_dir = output_dir or Path(f"./output/{exp_name}_noiseclouds_adaptrans")
    LoggingConfigurator(
        output_dir=_output_dir,
        log_filename="pipeline_noiseclouds_adaptrans.log",
    ...
    spont_rate_used = float(per_seq[NC_SILENCE_SEQ_ID]["train"][0])
    ...
    for combo in param_grid:
        ...
        saver.save_npz(
            save_dict,
            f"{exp_name}_noiseclouds_adaptrans_cf{cf:03d}{pref_str}{alpha_str}{combo_suffix}_bold.npz",
        )
