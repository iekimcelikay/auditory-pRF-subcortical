"""
cochlea_toneclouds_gaussianprf.py

Runs the Zilany2014 cochlear simulation on one tone-cloud WAV file from the
toneclouds_gaussianprf stimulus set (produced by
stimuli/save_tone_clouds_gaussian_prf.py).
...
-----
    ...
    python examples/cochlea_toneclouds_gaussianprf.py --wav-index 0

    # with explicit directory (defaults to latest toneclouds_gaussianprf_* run)
    python examples/cochlea_toneclouds_gaussianprf.py \
        --wav-index 5 \
        --wav-dir /scratch/ecelikay/workspace/auditory-pRF-subcortical/stimuli/produced/toneclouds_gaussianprf_20260612_0155

Filename convention (produced by save_tone_clouds_gaussian_prf.py):
    tonecloud{g:02d}_fc{center}hz_dur{D}ms_isi{I}ms_numtones{N}.wav
    tonecloud00_dur0ms_isi0ms.wav   (silence / null trial)
"""
...
def get_latest_produced_stimuli(prefix: str = "toneclouds_gaussianprf") -> Path:
    ...
    ----------
    prefix : str
        Directory name prefix to match (default: "toneclouds_gaussianprf").
...
def parse_tonecloud_filename(filename: str) -> dict:
    """Parse a tonecloud filename into metadata fields.
    ...
    filename : str
        ...
        ``tonecloud01_fc885hz_dur100ms_isi67ms_numtones100.wav``

    Returns
    -------
    dict
        Keys: sequence, center_freq, tone_duration, isi, num_tones.
    """
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Cochlear simulation for toneclouds_gaussianprf stimuli."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to stimulus directory. Defaults to latest toneclouds_gaussianprf_* run.",
    )
    ...
    config = CochleaConfig(
        ...
        experiment_name=f"toneclouds_gaussianprf_128ANF_wav{args.wav_index:03d}",
        save_formats=["npz"],
    ...
    simulation = CochleaWavSimulation(
        config, wav_file,
        auto_parse=True,
        parser_func=parse_tonecloud_filename,
    )
    simulation.run()
