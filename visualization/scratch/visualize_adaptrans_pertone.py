...
==============================
Visualise the AdapTrans ON filter applied per-tone for one stimulus sequence.

For each tone repetition in the sequence, an isolated boxcar train is built
from the pRF response scalar (sharpened & normalized 
-to the previous mean across CF- mean AN rate × duration Gaussian), 
passed through the AdapTrans ON filter, 
and superposed to produce the full ON timecourse.

Layout (N_tones + 2 rows):
  Row 0          : accumulated boxcar train (all tones summed, for reference)
  Rows 1–N_tones : per-tone isolated boxcar (shaded) + its ON response (line)
...
from auditory_prf.prf_pipeline.powerlaw_function import apply_powerlaw_cf, apply_powerlaw_population
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Visualise per-tone AdapTrans ON filter for one stimulus."
    )
...
def main():
    ...
    # ── 6-7. Per-tone AdapTrans ON, then superpose ────────────────────────────
    per_tone_trains   = []
    per_tone_on       = []
    ...
    for s in range(n_tones):
        ax = axes[s + 1]
        c  = colors[s]
        ax.fill_between(t_ms, per_tone_trains[s], alpha=0.25, color=c, linewidth=0)
