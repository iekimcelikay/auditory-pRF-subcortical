"""
plot_toneclouds_adaptrans_bold_timecourses.py
==============================================
Plot predicted BOLD timecourses for the tone-cloud Gaussian-pRF AdapTrans
pipeline output -- one figure per (param combo, run), each a grid of CF
panels, with a condition timeline strip on top showing which tone-cloud
band / duration was playing at each point in time, and the noisy variant
at a chosen noise level overlaid.

Usage
-----
    python visualization/scratch/plot_toneclouds_adaptrans_bold_timecourses.py
    python visualization/scratch/plot_toneclouds_adaptrans_bold_timecourses.py \\
        --bold_dir models_output/toneclouds_gaussianprf_20260612_0155/adaptrans_bold \\
        --cochlea_dir models_output/toneclouds_gaussianprf_20260612_0155 \\
        --noise_level mid \\
        --n_cols 5
"""
...
from auditory_prf.prf_pipeline.full_pipeline_toneclouds_adaptrans import (
    ...
    ITI_RANGE_S, BASE_SEED, TC_SILENCE_SEQ_ID,
    _make_tonecloud_seq_id_fn,
)

_COND_RE = re.compile(r'_fc(\d+)hz_dur(\d+)ms')
_FILE_RE = re.compile(r'cf(\d+)(_.*)_bold\.npz')


# ── data loading ──────────────────────────────────────────────────────────────

def load_bold_matrix(npz_path: Path, noise_level: str) -> tuple[np.ndarray, np.ndarray | None, int, float]:
    """Load all run timecourses for one (CF, combo) npz, plus the noisy variant.

    Returns
    -------
    bold_matrix       : np.ndarray, shape (n_runs, n_trs)
    bold_matrix_noisy : np.ndarray, shape (n_runs, n_trs), or None if
                         ``run_XX_noisy_<noise_level>`` keys are absent.
    cf_index          : int
    tr_s              : float
    """
    data = np.load(npz_path, allow_pickle=True)
    run_keys = sorted(
        (k for k in data.files if re.fullmatch(r'run_\d+', k)),
        key=lambda k: int(k.split("_")[1]),
    )
    bold_matrix = np.stack([data[k] for k in run_keys], axis=0)

    noisy_keys = [f"{k}_noisy_{noise_level}" for k in run_keys]
...
def parse_condition(seq_id: str) -> tuple[str, int | None]:
    """Return (band_label, duration_ms) for a tone-cloud seq_id, or ("null", None)."""
...
def plot_run_figure(
    ...
    """One figure for one (combo, run): condition timeline strip + grid of CF BOLD panels."""
    ...
    combo_label = combo_suffix.lstrip("_")
    ...
    ax_timeline.set_title(
        f"Run {run_idx + 1:02d} ({combo_label}) — condition timeline "
        f"(band color = freq, number = tone duration in ms)",
        fontsize=10,
    )
    legend_handles = [Patch(facecolor=c, alpha=0.6, label=lbl) for lbl, c in colors.items()]
    if cf_data_noisy:
        from matplotlib.lines import Line2D
        legend_handles.append(Line2D([0], [0], color="darkgray", lw=1.5,
                                      label=f"BOLD + noise ({noise_level})"))
    ...
    for idx, cf_index in enumerate(cf_indices):
        ...
        if cf_data_noisy is not None and cf_index in cf_data_noisy:
            ax.plot(time_s, cf_data_noisy[cf_index][run_idx],
                    color="lightgray", lw=0.8, zorder=1, label=f"BOLD + noise ({noise_level})")
    ...
    fig.suptitle(
        f"Tone-cloud Gaussian-pRF AdapTrans — predicted BOLD per CF ({combo_label})",
        fontsize=12, y=1.0,
    )
    fig.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot per-combo, per-run, per-CF AdapTrans BOLD timecourses "
                     "with condition timelines and a noisy overlay."
    )
    parser.add_argument(
        "--bold_dir", default="models_output/toneclouds_gaussianprf_20260619_0114/adaptrans_bold",
        help="Directory containing *_toneclouds_adaptrans_cf*_bold.npz files.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--noise_level", default="mid", choices=["low", "mid", "high"],
                         help="Noisy variant to overlay (run_XX_noisy_<level>). Default: mid.")
    ...
    npz_files = sorted(bold_dir.glob("*_toneclouds_adaptrans_cf*_bold.npz"))
    if not npz_files:
        print(f"No matching npz files in {bold_dir}")
        return
    print(f"Found {len(npz_files)} npz file(s).")

    # group files by param-combo suffix (e.g. "_tau035_w0p80_rho1p00")
    combo_files: dict[str, list[Path]] = defaultdict(list)
    skipped_files = []
    for npz_path in npz_files:
        m = _FILE_RE.search(npz_path.name)
        if m is None:
            skipped_files.append(npz_path.name)
            continue
        combo_files[m.group(2)].append(npz_path)
    if skipped_files:
        print(f"Skipping {len(skipped_files)} npz file(s) without a "
              f"'_tau..._w..._rho..._bold.npz' combo suffix (likely from "
              f"before the param-sweep redesign):")
        for name in skipped_files:
            print(f"  {name}")

    cf_hz_values = load_cf_hz_values(cochlea_dir)

    for combo_suffix, paths in sorted(combo_files.items()):
        print(f"Combo {combo_suffix}: {len(paths)} CF file(s).")
        cf_data = {}
        cf_data_noisy = {}
        tr_s = 1.6
        for npz_path in paths:
            bold_matrix, bold_matrix_noisy, cf_index, tr_s = load_bold_matrix(npz_path, args.noise_level)
        ...
        for run_idx in range(n_runs):
            plot_run_figure(
                ...
                n_cols       = args.n_cols,
                combo_suffix = combo_suffix,
                noise_level  = args.noise_level,
                save_path    = save_dir / f"toneclouds_adaptrans_bold_timecourses{combo_suffix}_run{run_idx + 1:02d}.png",
            )
