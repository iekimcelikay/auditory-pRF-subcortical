"""
plot_toneclouds_bold_timecourses.py
====================================
Plot predicted BOLD timecourses for the tone-cloud Gaussian-pRF notemporal
pipeline output -- one figure per run, each a grid of 30 CF panels, with a
condition timeline strip on top showing which tone-cloud band / duration was
playing at each point in time.

Usage
-----
    python visualization/scratch/plot_toneclouds_bold_timecourses.py
    python visualization/scratch/plot_toneclouds_bold_timecourses.py \\
        --bold_dir models_output/toneclouds_gaussianprf_20260612_0155/notemporal_bold \\
...
from auditory_prf.prf_pipeline.full_pipeline_toneclouds_notemporal import (
...
def plot_run_figure(
    ...
    fig.suptitle(
        "Tone-cloud Gaussian-pRF — predicted BOLD per CF",
        fontsize=12, y=1.0,
    )
    fig.tight_layout()
...
def main() -> None:
    ...
    parser.add_argument(
        "--bold_dir", default="models_output/toneclouds_gaussianprf_20260612_0155/notemporal_bold",
        help="Directory containing *_toneclouds_notemporal_cf*_bold.npz files.",
    )
    parser.add_argument(
    ...
    npz_files = sorted(bold_dir.glob("*_toneclouds_notemporal_cf*_bold.npz"))
    if not npz_files:
    ...
    for run_idx in range(n_runs):
        plot_run_figure(
            ...
            n_cols    = args.n_cols,
            save_path = save_dir / f"toneclouds_bold_timecourses_run{run_idx + 1:02d}.png",
        )
